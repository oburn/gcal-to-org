...
export default class Generate extends Command {
  ...
  static flags = {
    ...
    backDays: Flags.integer({ default: 30, description: "How many days back to process events." }),
    forwardDays: Flags.integer({ default: 60, description: "How many days forward to process events." })
  }

  static args = {
    file: Args.string({ description: 'Path to output the Org file (will be overwritten)', required: true }),
  }

  public async run(): Promise<void> {
    const { args, flags } = await this.parse(Generate)

    this.log(`generated ${args.file} going back ${flags.backDays} and forward ${flags.forwardDays} with port ${flags.port}`)
    ...
    * Google Calendar: primary
    ...
    `)
    orglines.forEach(line => outStream.write(`\n${line}\n`))
    outStream.close()
  }

  formatEntry(e: calendar_v3.Schema$Event): string {
  ...
  acceptedOrCreated(e: calendar_v3.Schema$Event): boolean {
