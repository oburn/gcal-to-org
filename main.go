...
func warningEvent(event *calendar.Event) string {
	...
	if selfFor(event).ResponseStatus == "needsAction" {
...
func relevantEvent(event *calendar.Event) bool {
	...
	for _, attendee := range event.Attendees {
		if attendee.Self && (attendee.ResponseStatus == "tentative" || attendee.ResponseStatus == "declined") {
			return false
		}
	}
